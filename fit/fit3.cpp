/*
   c++ -o fit3 fit3.cpp `root-config --cflags --glibs` ../lib/CfgParser.cc -lm
 */
...
int main(int argc, char** argv)
{
        ...
        string n3[] = {"N", "f_{\\mu^+}", "\\tau_{\\mu^-}", "\\tau_{\\mu^+}", "Bkg"};
        if (function_type==1){
        ...
        if (name == "cerbero_sopra_new_CORR_down_two_exp"){
            ...
            TFitResultPtr r = h->Fit("fun","SR");
            // "  \\tau_{\mu^+}"
            h->SetTitle((title + "  f_{\mu^+}").c_str());
            cnv->Modified();
            cnv->Update();

            cnv->Print(("images/"+trimTitle(title)+"_f_muplus_newStudy.png").c_str(), "png");


            double n_minus =  func->GetParameter(1);
            double tau_minus = func->GetParameter(3);
            double n_minus_err = r->ParError(1);
            double tau_minus_err = r->ParError(3);


            double tau_minus_teor = 2.025;
            double tau_minus_teor_err = 0.004;


            double tau_plus_teor = 2.1969811;
            double tau_plus_teor_err = 2.2e-6;


            double t = abs(tau_minus - tau_plus_teor)/sqrt(pow(tau_minus_err,2) + pow(tau_plus_teor_err,2));
            ...
            cout << "f_mu_+ - ricavato: " << n_minus << " ± " << n_minus_err <<endl;
